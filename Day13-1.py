...
def solve(data):
    ...
    for i in range(0, len(lines), 4):
        ...
        B = lines[i + 1].split()
        P = lines[i + 2].split()
        machines.append([(int(A[2][2:-1]), int(A[3][2:])), 
                         (int(B[2][2:-1]), int(B[3][2:])), 
                         (int(P[1][2:-1]), int(P[2][2:]))])
    dp(machines)
...
def dp(games):
    ...
    for game in games:
        ...
        BX, BY = game[1]
        X, Y = game[2]
        for b in range(101): ## Cost of 1
            for a in range(101): ## Cost of 3
                if (BX * b + AX * a == X and BY * b + AY * a == Y):
                    tokens += b + a * 3
        print(tokens)
main()
